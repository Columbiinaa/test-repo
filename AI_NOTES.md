...
* **Database & ORM:** Prisma ORM, SQLite (for rapid local prototyping), Neon (Serverless PostgreSQL for production).
...
1. **The Database Crash:** When deleting the local `dev.db` file to migrate to PostgreSQL, Prisma crashed because the `.env` string was still formatted for SQLite, throwing a validation error. I diagnosed this by checking the terminal traces and updated the `DATABASE_URL` to a properly formatted, URL-encoded Neon PostgreSQL connection string.
